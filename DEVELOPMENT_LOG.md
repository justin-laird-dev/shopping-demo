...
Before committing, a full review pass against all four artifacts caught a real, worth-remembering problem: several corrections agreed to earlier in conversation (dropping a confidence field, splitting one API call into two for honest progress feedback, a modal instead of a page, a tabular "pre-cart" with a single accept action) had been verbally agreed to but never actually applied to the files — a long conversation's spoken agreement is not the same thing as it being in the written artifact. Catching this here, before the spec was committed and handed to a fresh `apply` session with no memory of the conversation, is exactly why the review step exists; it would otherwise have shipped as silent scope drift. The fix cascaded further than expected too: splitting the API into two endpoints broke an earlier single-graph design, which needed its own follow-up correction — a small reminder that "add" reviews should also check what a change ripples into, not just the lines directly touched.
